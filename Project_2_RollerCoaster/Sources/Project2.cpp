...
int main()
{
	...
	/*std::vector<std::string> faces =
	{
		"../Project_2/Media/skybox/right.png",
		"../Project_2/Media/skybox/left.png",
		"../Project_2/Media/skybox/top.png",
		"../Project_2/Media/skybox/bottom.png",
		"../Project_2/Media/skybox/back.png",
		"../Project_2/Media/skybox/front.png"
	};*/
	unsigned int cubemapTexture = loadCubemap(faces);

	// init heatmap
	Heightmap heightmap("../Project_2/Media/heightmaps/hflab4.jpg");
	unsigned int heightmap_texture = loadTexture("../Project_2/Media/heightmaps/grass.jpg");
	...
	unsigned int firetexture = loadTexture("../Project_2/Media/textures/fire.jpg");
	...
	Model ourModel("../Project_2/Media/cart/roller.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Draw the track
		track.Draw(reflectionShader, heightmap_texture);


		
		Orientation camera_ori = track.get_camera_orentation(new_s);
		Orientation new_ori;

		new_vec3 = camera_ori.origin;


		//ourModel.postion
		
		new_ori.origin = camera_ori.origin + camera_ori.Up * 0.05f - camera_ori.Right * 0.3f + camera_ori.Front * 0.3f;
		new_ori.Front = camera_ori.Front;
		new_ori.Up = camera_ori.Up;
		new_ori.Right = camera_ori.Right;
		...
		model = glm::translate(model, glm::vec3(new_ori.origin)); // translate it down so it's at the center of the scene
		model[0][0] = new_ori.Right.x;
		model[0][1] = new_ori.Right.y;
		model[0][2] = new_ori.Right.z;
		model[1][0] = new_ori.Up.x;
		model[1][1] = new_ori.Up.y;
		model[1][2] = new_ori.Up.z;
		model[2][0] = new_ori.Front.x;
		model[2][1] = new_ori.Front.y;
		model[2][2] = new_ori.Front.z;
		...
		if (toggleCamera == FALSE) {
			...
			camera.Position = camera_ori.origin + camera_ori.Up * 0.7f - camera_ori.Right * 0.2f - camera_ori.Front * 0.9f;
			camera.Front = camera_ori.Front;
			camera.Up = camera_ori.Up;
			camera.Right = camera_ori.Right;
		...
		float delta_x = track.distance_moved(deltaTime/3, prevous_velocity);
		//printf("delta_x = %f, new_velocity = %f, deltaTime = %f\n", delta_x, prevous_velocity, deltaTime);

		new_s += delta_x;
		if (new_s <= 0.0f || new_s >= track.return_track_length() - 3) { new_s = 1.0f; }

		float delta_y = track.height_diffrence(old_vec3, new_vec3);
		prevous_velocity = track.velocity(delta_y, prevous_velocity);
		if (prevous_velocity > 1.0f && prevous_velocity < 100.0f) {
			// do nothing
		}
		else {
			//printf("sooo slow  enabling boost");
			prevous_velocity = 1.0f;
		}
		printf("old_position = %f,  camera.Position = %f\n", old_vec3.y, new_vec3.y);
		printf("delta_y = %f,  velocity = %f\n", delta_y, prevous_velocity);
		...
		if (drawNormals)
		{
			...
			track.Draw(normalShader, heightmap_texture);
		}
...
void processInput(GLFWwindow *window)
{
	...
	if (somethingPressed && last_pressed < currentFrame - 0.5f || last_pressed == 0.0f)
	{
		...
		if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS)
			toggleCamera ? toggleCamera = false : toggleCamera = true;
